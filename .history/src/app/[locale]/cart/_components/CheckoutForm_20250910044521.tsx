...
function CheckoutForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedPaymentMethod) return toast({description:'Please choose a payment method'});
    ...
    try {
      if (selectedPaymentMethod === 'cod') {
        // Cash on Delivery
        // Map formDataObj to FormDataType
        const mappedFormData = {
          name: (formDataObj.userEmail as string) || '',
          address: (formDataObj.streetAddress as string) || '',
          streetAddress: (formDataObj.streetAddress as string) || '',
          email: (formDataObj.userEmail as string) || '',
      ...
      console.error(err);
      toast({ description: 'Failed to place order.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  <div className="grid gap-1">
    ...
    />
            ...
            <div className='grid gap-1'>
              ...
              <Textarea id='address' placeholder='Enter your address' name='streetAddress' className='resize-none' required />
            </div>
