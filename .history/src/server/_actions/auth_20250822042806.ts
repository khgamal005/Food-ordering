...
export const login = async (
  ...
  if (!credentials?.email || !credentials?.password) {
    return {
      error: {
        email: !credentials?.email ? translations.validation.passwordMinLength : undefined,
        password: !credentials?.password ? translations.validation.passwordMaxLength : undefined,
      },
      status: 400,
    };
  }
