...
authorize: async (credentials, req) => {
  ...
  // Pass error details via null + custom message
  // NextAuth will treat this as invalid credentials
  throw new Error(
    JSON.stringify({
      validationError: res.error ?? null,
      responseError: res.message ?? "Invalid credentials",
    })
  );

  // OR safer:
  // return null; // and handle generic error in client
}
  ],
